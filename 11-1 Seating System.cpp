...
int check_seats_around(vector<vector<char>> &seats, char symbol, int i, int j)
{
    // i --- y_diag
    // j --- x_diag
    int counter = 0;
    if (seats[i - 1][j] == symbol)
    {
        counter++;
    }
    if (seats[i - 1][j + 1] == symbol)
    {
        counter++;
    }
    if (seats[i][j + 1] == symbol)
    {
        counter++;
    }
    if (seats[i + 1][j + 1] == symbol)
